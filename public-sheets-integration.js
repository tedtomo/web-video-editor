...
class PublicSheetsIntegration {
  ...
  async clearExecutionFlag(spreadsheetId, rowIndex) {
    console.log(`ℹ️ CSV方式では行${rowIndex}の実行フラグをクリアできません（手動でスプレッドシートから○を削除してください）`);
    return { updated: false, message: 'CSV方式では更新不可' };
  }

  async recordVideoUrl(spreadsheetId, rowIndex, videoUrl) {
    console.log(`ℹ️ CSV方式では行${rowIndex}に動画URL（${videoUrl}）を記録できません（手動でスプレッドシートに貼り付けてください）`);
    return { updated: false, message: 'CSV方式では更新不可', videoUrl };
  }
}
